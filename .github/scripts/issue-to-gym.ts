...
if (!coords) {
  console.error('::error::Could not parse coordinates. Paste a Google Maps link, or two numbers "lat, lng".');
  process.exit(1);
}
...
const affiliationName = value(sections, 'Affiliation (optional)');
const gym: Gym = {
  id: newObjectId(),
  name,
  ...
  affiliation: affiliationName ? { name: affiliationName, website: null } : null,
  trialOffer,
  location: {
    address,
    venue: name,
...
const outPath = path.join('seeder/data/gyms', `${slug}.json`);
