...
import type { BjjEvent } from './types.ts';
...
const event: BjjEvent = {
  ...
  // validate-data (strict deserialization) will surface any mismatch for a
  // maintainer to clean up before merge.
...
const outPath = path.join('seeder/data/bjj-events', `${slug}.json`);
