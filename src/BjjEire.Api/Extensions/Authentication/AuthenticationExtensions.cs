...
public static class AuthenticationExtensions
{
    ...
    private static void ValidateJwtOptions(JwtOptions options, ILogger? logger)
    {
        ...
        if (string.IsNullOrWhiteSpace(options.Issuer)) validationErrors.Add($"{nameof(options.Issuer)} is missing.");
        if (string.IsNullOrWhiteSpace(options.Audience)) validationErrors.Add($"{nameof(options.Audience)} is missing.");
    ...
    {
        var validationErrors = new List<string>();
        if (string.IsNullOrWhiteSpace(options.HeaderName)) validationErrors.Add($"{nameof(options.HeaderName)} is missing.");
        if (string.IsNullOrWhiteSpace(options.ApiKeyValue)) validationErrors.Add($"{nameof(options.ApiKeyValue)} is missing.");
