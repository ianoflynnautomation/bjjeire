...
export const GymsPage: React.FC = () => {
  ...
  const paginationInfo = useMemo(() => paginatedGyms?.pagination, [paginatedGyms?.pagination]);
  ...
  const noGymsFound = !isInitialLoading && !hasFetchError && gyms.length === 0 && !isFetching;
  ...
  const fetchErrorMessage = useMemo(() => {
    ...
    if (fetchError instanceof Error) {
      return fetchError.message?.includes('failed to fetch') ||
        ...
        : fetchError.message || 'An unexpected error occurred while loading gyms. Please try again.';
    }
  ...
  const renderMainContent = () => {
    if (isInitialLoading) {
      return <LoadingState skeletons={<GymCardSkeletons />} />;
    }
    if (hasFetchError) {
      return <ErrorState message={fetchErrorMessage} onRetry={handleRetryFetch} />;
    }
    if (noGymsFound) {
      return (
        <NoDataState
  ...
  return (
    <div className="min-h-screen dark:bg-slate-900 sm:py-12">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <GymsPageHeader countyName={countyLabel} totalGyms={paginationInfo?.totalItems} />
        ...
        <main className="relative" aria-live="polite" aria-busy={isFetching}>
          {isBackgroundFetching && gyms.length > 0 && <BackgroundFetchingIndicator />}
          {renderMainContent()}
        </main>

        {paginationInfo && paginationInfo.totalPages > 1 && !hasFetchError && gyms.length > 0 && (
