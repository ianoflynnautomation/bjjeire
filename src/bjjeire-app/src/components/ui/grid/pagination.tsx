...
const Pagination = function Pagination({
  ...
  const itemsText =
    itemsStart && itemsEnd
      ? `${paginationContent.showingPrefix} ${itemsStart}-${itemsEnd} ${paginationContent.ofSeparator} ${totalItems} ${paginationContent.itemsSuffix}`
      : ''
  ...
  return (
    <nav
      ...
      aria-label={paginationContent.navAriaLabel}
    >
      {itemsText && (
        <p
          data-testid={PaginationTestIds.ITEMS_TEXT}
          className="inline-flex items-center rounded-full bg-muted px-3 py-1 text-xs font-medium tabular-nums text-fg-muted ring-1 ring-hairline"
          aria-live="polite"
        >
          {itemsText}
        </p>
      ...
      <div className="flex items-center gap-2">
        ...
        <span
          ...
          aria-current="page"
          aria-label={`${paginationContent.pageAriaPrefix} ${currentPage} ${paginationContent.ofSeparator} ${totalPages}`}
        >
