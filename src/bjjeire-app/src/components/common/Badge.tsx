...
interface BadgeProps {
  text: string;
  colorScheme?: 'emerald' | 'slate' | 'amber' | 'red' | 'blue' | 'neutral'; // Add more as needed
  size?: 'xs' | 'sm';
  className?: string;
  'data-testid'?: string;
}
...
export const Badge: React.FC<BadgeProps> = ({
  ...
  const baseClasses = 'inline-flex items-center rounded-full font-semibold uppercase tracking-wider';
  ...
  return (
    <span
      className={clsx(baseClasses, sizeClasses[size], colorClasses[colorScheme], className)}
      data-testid={dataTestId}
    >
      {text}
    </span>
