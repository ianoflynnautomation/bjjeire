...
const ScheduleSection: React.FC<{ title: string; icon: React.ReactNode; children: React.ReactNode }> = ({
  title,
  icon,
  children,
}) => (
  <div className="space-y-3">
    <div className="flex items-center gap-x-2">
      <span className="h-5 w-5 text-slate-400" aria-hidden="true">{icon}</span>
...
export const EventSchedule: React.FC<EventScheduleProps> = ({ schedule }) => {
    ...
    <ul className="mt-1 list-none space-y-0.5 pl-5">
      {hours.slice(0, 3).map((hour, index) => (
        <ScheduleItem key={index}>
          {scheduleType === ScheduleType.Recurring && hour.dayOfWeek !== null && hour.dayOfWeek !== undefined
      ...
      {hours.length > 3 && (
        <li className="pl-0 text-xs text-slate-600 italic">...and more</li>
      )}
    </ul>
  );
  ...
  return <EmptyScheduleMessage message={`Invalid or unsupported schedule type: ${schedule.scheduleType}.`} />;
};