...
Backend tests (JUnit + Testcontainers — requires Docker for the ITs):
```
$(!mvn -pl src/bjjeire-api verify 2>&1 | grep -E "Tests run|BUILD|ERROR" | tail -30)
```

Summarise:
- Total passed / failed per suite (unit / integration / browser / java)
