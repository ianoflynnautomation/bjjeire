...
- Connection string is injected via the `MONGODB_URI` environment variable — never hardcoded in `application.yml`
...
- All queries use Spring Data MongoDB repositories or typed `Criteria`/`Query` objects — never build BSON/JSON query strings from user input
- Derived query methods and typed `Criteria.where(field)` are safe — string-concatenated filters are not
- Field names in updates are compile-time constants — never pass user input as a field name
...
- Seeder Docker image is separate (`src/bjjeire-api/seeder.Dockerfile`) — do not include it in the `app` compose profile
...
- Use a specific MongoDB image tag (`mongo:8.2`) — never `mongo:latest` in production
