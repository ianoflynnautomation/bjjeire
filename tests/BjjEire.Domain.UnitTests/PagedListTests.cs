...
public sealed class PagedListTests
{
    ...
    {
        PagedList<int> sut = Build(count: 10, pageIndex: 2, pageSize: 3);
    ...
    {
        PagedList<int> sut = Build(count: 10, pageIndex: 0, pageSize: 5);
    ...
    {
        PagedList<int> sut = Build(count: 42, pageIndex: 0, pageSize: 10);
    ...
    {
        PagedList<int> sut = Build(count, pageIndex: 0, pageSize);
    ...
    {
        PagedList<int> sut = Build(count: 0, pageIndex: 0, pageSize: 10);
    ...
    {
        PagedList<int> sut = Build(count: 10, pageIndex: 0, pageSize: 0);
        sut.TotalPages.ShouldBe(0);
    }
    ...
    {
        PagedList<int> sut = Build(count: 20, pageIndex: 0, pageSize: 10);
    ...
    {
        PagedList<int> sut = Build(count: 100, pageIndex, pageSize: 10);
    ...
    public void HasNextPage_WhenOnLastPage_ReturnsFalse()
    {
        ...
        PagedList<int> sut = Build(count: 10, pageIndex: 1, pageSize: 5);
    ...
    {
        List<int> items = Enumerable.Range(1, Math.Min(count, pageSize > 0 ? pageSize : count)).ToList();
