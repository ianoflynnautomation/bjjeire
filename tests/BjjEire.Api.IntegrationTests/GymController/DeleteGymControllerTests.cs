    ...
    : ParallelTestBase(fixture, output)
{
    ...
    public async Task DeleteGym_ConcurrentRequests_ShouldHandleCorrectlyAsync()
    {
        ...
        // Assert
        // Exactly one request wins the delete; the others get either:
        //   409 Conflict  — received cached entity but lost the DB race (ConcurrencyException)
        //   404 NotFound  — cache already invalidated; GetById returned null
        // The split is timing-dependent so only the total is asserted.
